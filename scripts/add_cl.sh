...
# Перезаписываем значение переменной vap_ip_local в файле /etc/wgtginst/scripts/variables.sh
grep -q "vap_ip_local=" /etc/wgtginst/scripts/variables.sh && sed -i "s/vap_ip_local=.*/vap_ip_local=${vap_ip_local}/" /etc/wgtginst/scripts/variables.sh || echo "vap_ip_local=${vap_ip_local}" >> /etc/wgtginst/scripts/variables.sh
