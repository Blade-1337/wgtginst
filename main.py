...
def del_vpn(message):
    if message.sticker is not None:
        ...
        if check_number_in_range(message.text):
            subprocess.run(['/etc/wgtginst/scripts/del_cl.sh', config_string])
...
#            bot.reply_to(message, 'Вы отправили текстовое сообщение.')
            config_string = check_message(message.text)
            subprocess.run(['/etc/wgtginst/scripts/add_cl.sh', config_string])
...
#                bot.send_message(message.chat.id, text="Привет хозяин")
            ...
            config_file_path_txt = f"/etc/wgtginst/cofigs.txt"
...
#                bot.send_message(message.chat.id, text="Привет хозяин")
                ...
                subprocess.run(['/etc/wgtginst/scripts/start_wg.sh'])
